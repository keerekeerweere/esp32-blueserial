...
static AsyncBluetoothSerialDataCb custom_data_callback = NULL;
...
void AsyncBluetoothSerial::onData(AsyncBluetoothSerialDataCb cb) {
...
static bool yieldForConnect(int timeout, std::function<void()> callback) {
  //let's keep the watchdog low
    //group this into (very) small chunks
  int chunkTimeout = timeout / 500;

  for (int i=0;i<=chunkTimeout;i++) {
    log_d("wait connection (%d / %d)", i, chunkTimeout);
    TickType_t xTicksToWait = chunkTimeout / portTICK_PERIOD_MS;
...
static bool waitForConnect(int timeout) {
  return yieldForConnect(timeout, (std::function<void()>)nullptr);
}
 ...
 *
 */
AsyncBluetoothSerial::AsyncBluetoothSerial() {
  local_name = "ESP32";  //default bluetooth name
}

AsyncBluetoothSerial::~AsyncBluetoothSerial(void) {
  _stop_bt();
}
 ...
 */
bool AsyncBluetoothSerial::begin(String localName, bool isMaster, bool disableBLE) {
  _isMaster = isMaster;
  if (localName.length()) {
...
int AsyncBluetoothSerial::available(void) {
  if (_spp_rx_queue == NULL) {
    return 0;
  }
...
int AsyncBluetoothSerial::peek(void) {
  uint8_t c;
...
bool AsyncBluetoothSerial::hasClient(void) {
  return _spp_client > 0;
}

int AsyncBluetoothSerial::read() {
 ...
 */
void AsyncBluetoothSerial::setTimeout(int timeoutMS) {
  Stream::setTimeout(timeoutMS);
  this->timeoutTicks = timeoutMS / portTICK_PERIOD_MS;
}

size_t AsyncBluetoothSerial::write(uint8_t c) {
  return write(&c, 1);
}

size_t AsyncBluetoothSerial::write(const uint8_t *buffer, size_t size) {
  if (!_spp_client) {
    return 0;
  }
...
void AsyncBluetoothSerial::flush() {
  if (_spp_tx_queue != NULL) {
 ...
 */
void AsyncBluetoothSerial::memrelease() {
  esp_bt_mem_release(ESP_BT_MODE_BTDM);
}

void AsyncBluetoothSerial::onConfirmRequest(ConfirmRequestCb cb) {
  confirm_request_callback = cb;
}

void AsyncBluetoothSerial::onKeyRequest(KeyRequestCb cb) {
  key_request_callback = cb;
}

void AsyncBluetoothSerial::respondPasskey(uint32_t passkey) {
  esp_bt_gap_ssp_passkey_reply(current_bd_addr, true, passkey);
}

void AsyncBluetoothSerial::onAuthComplete(AuthCompleteCb cb) {
  auth_complete_callback = cb;
}

void AsyncBluetoothSerial::confirmReply(boolean confirm) {
  esp_bt_gap_ssp_confirm_reply(current_bd_addr, confirm);
}

esp_err_t AsyncBluetoothSerial::register_callback(esp_spp_cb_t callback) {
...
void AsyncBluetoothSerial::enableSSP() {
...
void AsyncBluetoothSerial::enableSSP(bool inputCpability, bool outputCapability) {
...
// This must be called before calling begin, otherwise has no effect!
void AsyncBluetoothSerial::disableSSP() {
  _enableSSP = false;
}

bool AsyncBluetoothSerial::setPin(const char *pin, uint8_t pin_code_len) {
...
bool AsyncBluetoothSerial::connect(String remoteName) {
  bool retval = false;
 ...
 * @param callback while waiting for connection, use the callback (e.g. watchdog purposes)
 * @param sec_mask
 ...
 */
bool AsyncBluetoothSerial::connect(uint8_t remoteAddress[], std::function<void()> callback, int channel, esp_spp_sec_t sec_mask, esp_spp_role_t role) {
 bool retval = false;
  if (!isReady(true, READY_TIMEOUT)) {
    return false;
  }
  ...
  if (channel > 0) {
    ...
    if (esp_spp_connect(sec_mask, role, channel, _peer_bd_addr) != ESP_OK) {
      ...
      retval = yieldForConnect(READY_TIMEOUT, callback);
      if (retval) {
        log_i("connected");
      } else {
        if (this->isClosed()) {
 ...
 *           ESP_SPP_ROLE_SLAVE    can only have one connection to a master
 */
bool AsyncBluetoothSerial::connect(uint8_t remoteAddress[], int channel, esp_spp_sec_t sec_mask, esp_spp_role_t role) {
  return connect(remoteAddress, nullptr, channel, sec_mask, role);
}

bool AsyncBluetoothSerial::connect() {
  if (!isReady(true, READY_TIMEOUT)) {
    return false;
  }
  _doConnect = true;
...
bool AsyncBluetoothSerial::disconnect() {
  if (_spp_client) {
    flush();
    log_i("disconnecting");
...
bool AsyncBluetoothSerial::unpairDevice(uint8_t remoteAddress[]) {
  if (isReady(false, READY_TIMEOUT)) {
...
bool AsyncBluetoothSerial::connected(int timeout) {
 ...
 */
bool AsyncBluetoothSerial::isClosed() {
  return xEventGroupGetBits(_spp_event_group) & SPP_CLOSED;
}

bool AsyncBluetoothSerial::isReady(bool checkMaster, int timeout) {
 ...
 */
BTScanResults *AsyncBluetoothSerial::discover(int timeoutMs) {
  scanResults.clear();
 ...
 */
bool AsyncBluetoothSerial::discoverAsync(BTAdvertisedDeviceCb cb, int timeoutMs) {
  scanResults.clear();
...
void AsyncBluetoothSerial::discoverAsyncStop() {
  esp_bt_gap_cancel_discovery();
  advertisedDeviceCb = nullptr;
}

/** @brief      Clears scanResult entries */
void AsyncBluetoothSerial::discoverClear() {
  scanResults.clear();
}
 ...
 */
BTScanResults *AsyncBluetoothSerial::getScanResults() {
  return &scanResults;
}

AsyncBluetoothSerial::operator bool() const {
  return true;
}
 ...
 */
std::map<int, std::string> AsyncBluetoothSerial::getChannels(const BTAddress &remoteAddress) {
 ...
 */
void AsyncBluetoothSerial::getBtAddress(uint8_t *mac) {
 ...
 */
BTAddress AsyncBluetoothSerial::getBtAddressObject() {
 ...
 */
String AsyncBluetoothSerial::getBtAddressString() {
...
void AsyncBluetoothSerial::requestRemoteName(uint8_t remoteAddress[]) {
  if (isReady(false, READY_TIMEOUT)) {
...
bool AsyncBluetoothSerial::readRemoteName(char rmt_name[ESP_BT_GAP_MAX_BDNAME_LEN + 1]) {
  if (_rmt_name_valid) {
...
void AsyncBluetoothSerial::invalidateRemoteName() {
  _rmt_name_valid = false;
}

int AsyncBluetoothSerial::getNumberOfBondedDevices() {
...
int AsyncBluetoothSerial::getBondedDevices(uint dev_num, esp_bd_addr_t *dev_list) {
...
bool AsyncBluetoothSerial::deleteBondedDevice(uint8_t *remoteAddress) {
...
void AsyncBluetoothSerial::deleteAllBondedDevices() {
  if (!isReady(false, READY_TIMEOUT)) {
