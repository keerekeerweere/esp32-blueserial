...
typedef std::function<void(const uint8_t *buffer, size_t size)> AsyncBluetoothSerialDataCb;
...
class AsyncBluetoothSerial : public Stream {
public:
  AsyncBluetoothSerial(void);
  ~AsyncBluetoothSerial(void);
  ...
  void onData(AsyncBluetoothSerialDataCb cb);
  ...
  bool connect(
    uint8_t remoteAddress[], std::function<void()> callback, int channel = 0, esp_spp_sec_t sec_mask = (ESP_SPP_SEC_ENCRYPT | ESP_SPP_SEC_AUTHENTICATE),
    esp_spp_role_t role = ESP_SPP_ROLE_MASTER
  );
  bool connect(
